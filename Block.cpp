...
void Block::rotate(int gyro){
  ...
  if(nowblock>0){
  ev3_motor_rotate(EV3_PORT_C,angle,7,false);//ブロック所持
  ev3_motor_rotate(EV3_PORT_B,-angle,7,true);
  }
  else{
  ev3_motor_rotate(EV3_PORT_C,angle,15,false);//ブロック非所持
...
/*ブロック設置*/
  void Block::liftdown(){
    ...
    /*else if(goalflag>1&&black_flag==0){//黒を運んでない時
      red_flag=1;
      yellow_flag=1;
      blue_flag=1;
      green_flag=1;
