...
int ETsumou::PlayETsumou(){
	...
	**********************************/
	...
	//2列目へ移動
	while(local.GetY() < 305){			//交差点まで進んで停止
	...
	**********************************
	...
	/*4列目を無視して移動
	while(local.GetY() > -215){			//４列目交差点まで進んで停止
		linetrace.PIDSetRun(5,0.20,0,0.01,-1);
		//steer.setPower(30,0);
		local.SetPoint();
	}*/
	...
	/*壁が通り過ぎたら(3.4列目を無視、かつ２回目の線路横断で一番奥を新幹線が通過した時はぶつかる可能性がないため省略)
	if(wallflag <= 30){
		while(wall == 0){		
