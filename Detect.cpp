...
int Detect::WallDetection(int distance)
{
     ...
     if (sonarvalue <= distance){
        ...
        sonarflag_pre = sonarflag;
        sonarflag = 0;
     }

     fprintf(bt, "dis: %d, flag: %d\r\n",(int)sonar.getDistance(),sonarflag);
    
    if((sonarHistory[0] - sonarHistory[SONARDATA_N - 1]) == 1 && sonarflag_pre >= 700){//検出（１）→未検出（０）に変化した時、壁が通過したとみなす
        return 1;
    }
    else if(sonarflag >= 4000){  //センサの誤認識で値が閾値以下から動かなかった場合を検出
        clock.wait(500);
        return 1;
    } 
    else{ return 0; }
