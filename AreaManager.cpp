...
Parking:
				...
				if(detection == 0){
					...
					if(lotation == 0){
						...
						if(local.GetTheta() > 0 && count == 2){
							detection = 2;//ミスった時
						}
						else if(local.GetTheta() > 65){
							lotation = 1;
						}
					}
					else{
				...
				else if(detection == 2){//黒検知できなかった時
					
					if(local.GetX() < 340 && park == 0){
						leftWheel.setPWM(30);
        				rightWheel.setPWM(30);
						local.SetPoint();
					}

					else if(local.GetX() >= 340 && park == 0){
						local.Init();
						park = 1;
					}
					...
					if(local.GetX() < 340 && park == 0){
						...
						//linetrace.PIDSetRun(30, 0.30, 0.00, 0.040, -1);
						local.SetPoint();
					}
					else if(local.GetX() >= 340 && park == 0){
						local.Init();
...
End:
		...
		switch(currentstate){
			...
			case CURVE_L1:
				...
				linetrace.PIDSetRun(speed, 0.47, 0.03, 0.075, 1);//いいんじゃないか？
			...
			case CURVE_L2:
				...
				linetrace.PIDSetRun(speed, 0.25, 0.03, 0.050, 1);//良さげ 0.35, 0.50 折原
				//linetrace.PIDSetRun(speed, 0.38, 0.00, 0.065, 1);//良さげ 0.35, 0.50


				local.SetPoint();

				if(local.GetY() < /*250*/-600 && correct == 1){//-500
					//steer.setPower(0,0);
					//local.Init();
			...
			case CURVE_L3:
				...
				//else{
					...
					linetrace.PIDSetRun(speed, 0.50, 0.03, 0.075, 1);//0.47, 0.075ok
				//}

				local.SetPoint();

				if(local.GetY() > -1900){//-1650
					local.Init();
					currentstate = STRAIGHT_L2;
					/*linetrace.SetEdge(1);*/
				}
			...
			case STRAIGHT_L2:
				...
				if(black_on == 0){
					leftWheel.setPWM(35);//40
 				    rightWheel.setPWM(33);//38
 				    if(color.getColorNumber() == COLOR_BLACK){
 				    	clock.wait(100);
 				    	black_on = 1;
 				    	speed = 70;
					...
					if(second > 100){
						leftWheel.setPWM(33);//38
 				  		rightWheel.setPWM(35);//40
 				  		second--;
					}
					else{
...
Parking_L:
				...
				if(local.GetX() < 320 && park_l == 0){//350
					leftWheel.setPWM(30);//20
        			rightWheel.setPWM(30);
					local.SetPoint();
				}
				else if(local.GetX() >= 320 && park_l == 0){
					/*local.Init();
					park_l = 1;*/
					leftWheel.setPWM(-20);
        			rightWheel.setPWM(20);
...
void AreaManager::regularLine(){
	//if(local.GetLen() < 300){
		...
		linetrace.PIDSetRun(25, 0.330, 0, 0.060, 1);
		//local.SetPoint();
	//}
	//else{
		//local.Init();
